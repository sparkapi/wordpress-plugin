...
class ListingDetail extends Page {
	...
	function wp(){
		...
		$sf = $this->listing[ 'StandardFields' ];
		$this->listing[ 'StandardFields' ] = array_filter( $sf, function( $v ){
			if( is_array( $v ) ){
				return $v;
			}
			return !empty( $v ) && false === strpos( $v, '********' );
		} );
	...
	function wpseo_metadesc( $desc ){
		if( isset( $this->listing[ 'StandardFields' ][ 'PublicRemarks' ] ) ){
			return $this->listing[ 'StandardFields' ][ 'PublicRemarks' ];
		}
		$address = \FBS\Admin\Utilities::format_listing_street_address( $this->listing );
